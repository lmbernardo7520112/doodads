...
| **Botão "Já enviei o Pix" no card do cliente** | `auditoria_implementacao_pagamento.md` | Nenhuma na branch `main` | **PLANEJADO / NÃO CONFIRMADO** | Classificar como funcionalidade não integrada à branch estável (`main`). | Mover para a seção de melhorias planejadas, alertando que a funcionalidade aguarda fusão e validação. |
| **Status `manual_review` ("Em análise manual")** | `auditoria_implementacao_pagamento.md` | Enum em `BookingPayment.ts` e `Reserva.ts`, mas sem uso de rotas de transição | **PARCIALMENTE IMPLEMENTADO** | Classificar como estrutura presente no banco de dados, mas inativa operacionalmente no fluxo de checkout principal da `main`. | Indicar que a transição para este estado é planejada. |
