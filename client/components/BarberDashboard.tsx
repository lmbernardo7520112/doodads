...
// Phase E3: ConfirmModal, reserva status display, cancelled guard.
// =============================================================
...
export default function BarberDashboard() {
  ...
  // Ação: Marcar como expirado (via modal)
  const handleExpire = (p: any) => {
    setConfirmModal({
      isOpen: true,
      title: "Expirar Pagamento",
      message: "Deseja marcar este pagamento pendente vencido como expirado?\n\nEsta ação impedirá a confirmação futura deste pagamento.",
      confirmLabel: "Marcar como expirado",
      tone: "danger",
      onConfirm: async () => {
        setConfirmModal((prev) => ({ ...prev, isOpen: false }));
        try {
  ...
  return (
    <>
        ...
        {refreshing && pagamentos.length === 0 ? (
          ...
          <div className="space-y-4">
            {pagamentos.map((p) => {
              ...
              // Reserva status info
              const reservaStatus = p.reserva?.status || p.reservaStatus || null;
              const isReservaCancelled = p.isReservaCancelled || reservaStatus === "cancelado";

              return (
                <div
                  key={p.bookingPaymentId}
                  className={`bg-white p-4 rounded-xl border shadow-sm hover:shadow-md transition space-y-3 ${
                    isReservaCancelled ? "border-red-100 opacity-75" : "border-gray-100"
                  }`}
                >
                  {/* Reserva cancelada banner */}
                  {isReservaCancelled && (
                      ...
                      <div className="mt-1 flex flex-col gap-1 items-end">
                        ...
                        {reservaStatus && (
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium border ${
                            ...
                            Reserva: {reservaStatus === "cancelado" ? "Cancelada" :
                                      reservaStatus === "confirmado" ? "Confirmada" :
                                      reservaStatus === "pendente" ? "Pendente" :
                                      reservaStatus === "finalizado" ? "Finalizada" : reservaStatus}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  {/* Linha 3: Ações (bloqueadas para reservas canceladas) */}
                  {!isReservaCancelled && (p.canConfirm || p.canExpire) && (
                    <div className="pt-3 border-t border-gray-50 flex flex-wrap gap-2">
                      ...
                      {p.canExpire && (
                        <button
                          ...
                          className="flex-1 min-w-[150px] bg-red-600 hover:bg-red-700 text-white text-xs font-semibold py-2 px-4 rounded-lg transition"
                        >
      ...
      {/* Modal de confirmação/expiração */}
      <ConfirmModal
