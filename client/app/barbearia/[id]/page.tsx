...
export default function BarbeariaPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        console.log("📡 Buscando dados da barbearia e serviços...");
        const [barbRes, servRes] = await Promise.all([
          api.get(`/barbearias/${id}`),
          api.get(`/servicos/${id}`),
        ]);
        console.log("✅ Dados recebidos:", barbRes.data, servRes.data);
  ...
  // ==========================================================
  // 🧾 Função de reserva com logs e rastreio completo
  // ==========================================================
  const reservar = async (servico: Servico) => {
    try {
      ...
      console.log("🔑 Enviando reserva com token:", token);
      ...
      const response = await api.post(
        "/reservas",
        {
          barbearia: id,
          servico: servico._id,
          dataHora: new Date(Date.now() + 60 * 60 * 1000), // +1h
        },
        {
      ...
      if (err.response) {
        console.error(
          "❌ Erro ao reservar (resposta do backend):",
          err.response.status,
          err.response.data
        );
        alert(
        ...
        console.error("❌ Erro ao reservar (sem resposta):", err.request);
        alert("Sem resposta do servidor. Verifique a conexão com o backend.");
      } else {
        console.error("❌ Erro desconhecido ao reservar:", err.message);
